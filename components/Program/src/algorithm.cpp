...
const std::vector<std::string> Algorithm::_function_list = {
    "FlashDevice",
    "Init",
    "UnInit",
    "BlankCheck",
...
const uint32_t Algorithm::_flash_bolb_header[8] = {0xE00ABE00, 0x062D780D, 0x24084068, 0xD3000040, 0x1E644058, 0x1C49D1FA, 0x2A001E52, 0x4770D1F};

Algorithm::Algorithm()
{
}

bool Algorithm::read_elf_hdr(FILE *fp, Elf_Ehdr &elf_hdr)
{
    fseek(fp, 0, SEEK_SET);
...
bool Algorithm::find_shdr(FILE *fp, Elf_Ehdr &elf_hdr, Elf_Shdr &shstr_shdr, const std::string &scn_name, Elf_Shdr &shdr)
{
    long cur_pos = 0;
    std::string name;
...
bool Algorithm::find_shdr(FILE *fp, Elf_Ehdr &elf_hdr, Elf_Shdr &shstr_shdr, const std::string &scn_name, uint32_t type, Elf_Shdr &shdr)
{
    long cur_pos = 0;
    std::string name;
...
bool Algorithm::get_shstr_hdr(FILE *fp, Elf_Ehdr &elf_hdr, Elf_Shdr &shstr_shdr)
{
...
bool Algorithm::read_string(FILE *fp, Elf_Shdr &str_shdr, uint32_t offset, std::string &str)
{
    int c_tmp = 0;
    // FLM所用函数的名称的长度小于30字符
    char buf[300] = {0};
...
void Algorithm::read_symbol_info(FILE *fp, Elf_Shdr &string_hdr, Elf_Shdr &sym_hdr, Elf_Shdr &str_hdr, std::map<std::string, Elf_Sym> &sym)
{
    std::string name;
    long cur_pos = 0;
...
bool Algorithm::extract_flash_device(FILE *fp, Elf_Sym &sym, Elf_Shdr &shdr, FlashDevice &dev)
{
...
bool Algorithm::extract_flash_algo(FILE *fp, Elf_Shdr &code_scn, FlashIface::program_target_t &target)
{
...
bool Algorithm::extract(const std::string &path, FlashIface::program_target_t &target, FlashIface::target_cfg_t &cfg, uint32_t ram_start)
{
    bool ret = false;
    FILE *fp = nullptr;
    Elf_Ehdr elf_hdr;
